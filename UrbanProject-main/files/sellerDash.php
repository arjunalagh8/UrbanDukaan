...
<?php
	...
	$stmt0 = $conn->query("SELECT balance,name FROM urbandukan_project.seller where sid = $ownerSid;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT shopName,shopId FROM urbandukan_project.shop where ownerSid = $ownerSid;");
	while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		$shopId = $row['shopId'];
		$stmt1 = $conn->query("SELECT img1 FROM urbandukan_project.shopimage where shopId = $shopId;");
		$row1 = $stmt1->fetch(PDO::FETCH_ASSOC);
		if(!isset($row1['img1'])){
			$row1['img1']='Aroraa.jpg';
		}
		$image = $dir.'/'.$row1['img1'];
?>
