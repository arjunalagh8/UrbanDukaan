...
<?php
	...
	session_start();
	$cid = $_SESSION['NavCid'];
	$stmt0 = $conn->query("SELECT pid FROM urbandukan_project.cart where cid = $cid;");
	$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
	$stmt1 = $conn->query("SELECT name FROM urbandukan_project.customer where cid = $cid;");
...
<body>
  ...
  <div class="site-pusher">
	...
	<?php 
		$stmtit = $conn->query("SELECT count(*) as totalItems FROM urbandukan_project.cart where cid = $cid;");
		$stmtprice = $conn->query("SELECT sum(price) as Total FROM urbandukan_project.product P, urbandukan_project.cart A WHERE P.pid = A.pid AND A.cid= $cid;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT pid FROM urbandukan_project.cart where cid = $cid;");
	while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		$pid = $row['pid'];
		$stmt1 = $conn->query("SELECT productName,pid,img1,img2,img3,price FROM urbandukan_project.product where pid = $pid;");
		$row1=$stmt1->fetch(PDO::FETCH_ASSOC);
		
		$image = $dir.'/'.$row1['img1'];
		// $stmt2 = $conn->query("SELECT img1 FROM urbandukan_project.shopimage where shopId = $shopid;");
