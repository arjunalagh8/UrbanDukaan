...
<?php
	...
	$stmt0 = $conn->query("SELECT serName,shopId,category,baseCharge,img1,img2,info FROM urbandukan_project.service where serId = $serId;");

	$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
	$shopId = $row0['shopId'];
	$stmtshop = $conn->query("SELECT shopName,shopContact,shopEmail FROM urbandukan_project.shop where shopId = $shopId;");
