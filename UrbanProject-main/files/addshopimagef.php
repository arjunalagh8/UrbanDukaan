...
<?php
	...
	$delname='';
		$dir = 'uploads'; 
		$stmt = $conn->query("SELECT img1 FROM urbandukan_project.shopimage where shopId = $shopId;");
...
<?php
	...
	if($image==null){
		echo "<form action='file_uploaded.php' class='upload-form' method='POST'
			enctype='multipart/form-data' style='all:revert; text-align: center;' >
			<input type='file'  name='file' class='upload-file' data-max-size='2097152' required='required' style='margin-left: 77px;' >
			(Max-size 2MB)
...
if(isset($_POST['dsubmit'])){
		$sql = "DELETE FROM urbandukan_project.shopimage WHERE shopId =:c;";
...
$(function(){
    ...
    $('.upload-form').submit(function(e){
        if(fileInput.get(0).files.length){
            ...
            if(fileSize>maxSize){
                echo 'Error : image size is more then 2MB ';
                return false;
            }else{
